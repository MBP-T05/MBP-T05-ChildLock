...
def build_slide(slide):
    ...
    # Usually the slide master does this, but to be sure we can draw a big white rect
    ...
    # Subtle orange highlight line under header
    ...
    r2 = p2.add_run()
    r2.text = "[  PASSED  ] "
    r2.font.color.rgb = TERM_GREEN
    r2.font.name = 'Consolas'
    r2.font.size = Pt(11)
    
    r2b = p2.add_run()
    r2b.text = "140 tests.\n\n"
    r2b.font.color.rgb = TERM_WHITE
    r2b.font.name = 'Consolas'
    r2b.font.size = Pt(11)

    p3 = tf.add_paragraph()
    r3 = p3.add_run()
    r3.text = "Coverage: 100% (Branch)\n"
    r3.font.color.rgb = TERM_WHITE
    ...
    r4.font.color.rgb = TERM_WHITE
    r4.font.name = 'Consolas'
    r4.font.size = Pt(11)

    # Left Column: Sequential log output snippet
    log_y = term_y + term_h + 0.3
    log_h = 2.1
    add_rect(slide, left_x + term_pad, log_y, col_w - 2*term_pad, log_h, WHITE, border_color=BORDER_GRAY, rounded=False)
    ...
    r0.font.color.rgb = rgb(0x6c, 0x75, 0x7d)
    ...
    def add_log_line(tf, prefix, transition):
        ...
        r = p.add_run()
        r.text = prefix + " "
        r.font.color.rgb = rgb(0x00, 0x5b, 0xbb)
        r.font.name = 'Consolas'
        r.font.size = Pt(10)
        r2 = p.add_run()
        r2.text = transition
    ...
    add_tb(slide, right_x + gh_pad + 0.1, git_y + 0.1, col_w - 0.4, 0.2, "Branch Workflow", size=11, bold=True, color=DARK_TEXT)
    ...
    conn1 = slide.shapes.add_shape(MSO_SHAPE.LINE_INVERSE, Inches(right_x + gh_pad + 2.5), Inches(feat1_y), Inches(0.5), Inches(main_y - feat1_y))
    # Note: LINE_INVERSE draws line from top-left to bottom-right. We can use simple lines or just draw with rectangles since PPT coordinates are tricky.
    # Actually, a simple line shape:
    # 20 represents an angled connector
    # But for safety, I'll just use a thin rectangle for the diagonal, or just lines from (x,y) to (x+dx, y+dy)
    # Using add_connector is safer
    
    # Let's fix diagonal using a rectangle rotated, or just drawing a straight block down
    ...
    for i, nx in enumerate(nodes_x):
        ...
        if nx == 4.5: # final merge PR
            mpr = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(cx - 0.35), Inches(main_y - 0.4), Inches(0.7), Inches(0.22))
            mpr.fill.solid(); mpr.fill.fore_color.rgb = GREEN_SUCC; mpr.line.fill.background()
            add_tb(slide, cx - 0.35, main_y - 0.38, 0.7, 0.22, "Merge PR", size=8, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
